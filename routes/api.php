...
Route::group(['middleware' => 'api'], function() {
    ...
    // select cimeros.id, count(logros.cimero_id) as logros_count from cimeros left join logros on cimeros.id = logros.cimero_id where logros.provincia_id = 5 group by cimeros.id;
    ...
    // Filters cimeros by province or communidad
    Route::get('filtersranking/{filter}/{id}',function($filter,$id){
    	$cimeros = DB::table('cimeros')
	    	->select(DB::raw(
	    		'cimeros.*, 
	    		CONCAT(cimeros.nombre, " ", cimeros.apellido1, " ", COALESCE(cimeros.apellido2,"")) as fullName,
